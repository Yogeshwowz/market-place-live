...
class Home extends CI_Controller
{
    ...
    {
        $data['title'] = 'Home';
		/*if ($this->facebook->logged_in()){			$user = $this->facebook->user();			if ($user['code'] === 200){				$this->session->set_userdata('login',true);				$this->session->set_userdata('user_profile',$user['data']);							}		}else{			$data['link'] = $this->facebook->login_url();		}*/
	...
	public function paymentmethod(){
		...
		$this->load->view('dashboard/payment-method', $data);
		$this->load->view('template/frontend/footer');
	}
	
	
	public function loadRecordAdvisory($rowno=0){
	    $rowno =$_POST['pagno'];
		// Row per page
		$rowperpage = 5;
		// Row position
		if($rowno != 0){
      	...
      	// All records count
      	$allcount = $this->dashboard_model->getrecordCountAdvisory();
		// Get  records
      	$users_record = $this->dashboard_model->getDataAdvisory($rowno,$rowperpage);
      	
      	// Pagination Configuration
      	$config['base_url'] = base_url().'home/loadRecordAdvisory';
      	$config['use_page_numbers'] = TRUE;
		$config['total_rows'] = $allcount;
		$config['per_page'] = $rowperpage;
		$config['anchor_class'] = 'class="number" ';
