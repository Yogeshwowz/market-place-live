...
class Dashboard_model extends CI_Model{
   ...
   function removeimage(){
	...
	public function contactsetting(){
		$select =$this->users.".*,".$this->setting.".*,".$this->profile.".*";	   
		$this->db->select($select);
		$this->db->from($this->setting);
		$this->db->join($this->profile, $this->profile.'.user_id =' .$this->setting.'.uid','left');
		$this->db->join($this->users, $this->users.'.id =' .$this->setting.'.uid','left');
		$this->db->where($this->setting.'.uid', $this->session->userdata('uid'));
		$query = $this->db->get();
		return $query->row_array();
	 } 
    ...
    public function getrecordCountSeller() {
		...
		$where  =" where role=3 and ".$this->profile.".amount!=''";
    ...
    public function getrecordCountBuyer() {
		...
		$where  ="  where ".$this->profile.".amount!=''";
		if($_POST['tag']=='buyers'){
			$where  .=" and role=2 ";
		}else{
			$where  .=" and role=1 ";
	...
	// Fetch records Advisory
	 public function getrecordCountAdvisory() {
		...
		$where  ="  where  role=4 AND ".$this->profile.".amount!=''";
